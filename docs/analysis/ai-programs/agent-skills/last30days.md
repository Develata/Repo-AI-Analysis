...
作为 `ai-programs/agent-skills`，它比一般 prompt pack 更“工程化”：有 Python engine、tests、validate/security workflows、配置文档和明确的 host 安装路径；AGENTS.md 也强调“产品是 slash-command skill，CLI 只是 fallback” [Source:agents][Source:local-scan]。这使它具备真实工具属性，而不是一份静态提示词。
...
上述项目按 `ai-programs/agent-skills` 同类范围做定位级对比；Superpowers 与 Agent Skills 基于本地 wiki 已收录条目作参照，本条目未重新按同一 10 维度深审它们。

## 个人主页 sharing 候选

**是**。

- novelty：它把“最近 30 天社区真实信号”做成 agent skill，而不是普通搜索页。
- practicality：Develata 常做文献、热点、工具与主页分享，recency research 很贴合。
- shareability：故事线清楚——“Google 搜网页，last30days 搜人群最近的注意力”。
- caveat：分享时要强调它发现的是 engagement/sentiment，不是事实终审。
