...
- novelty：AG-UI + Generative UI 把 agent 产品化问题从“会不会推理”转到“怎么与用户共同操作”。
- practicality：前端/全栈开发者很容易理解其价值。
- maturity：项目 2023 年创建、33k stars、持续 release，但 backlog 高，分享时要带 caveat。
- narrative：可以作为“agent 应用真正难的是 UX/control surface”这条分享线的核心案例。
