...
evidence: "Current GitHub API/README + official docs + npm/PyPI metadata; historical 2026-05 repository local scan retained; not deployed in production by Develata"
archived_reason: ""
docker_support: true
gpu_required: false
estimated_cpu: "low-to-moderate for SDK/local vector-store prototyping; reference self-hosted server depends on REST API/Postgres/pgvector/dashboard load"
...
sources:
  ...
  - "[GH:api] https://api.github.com/repos/mem0ai/mem0 queried 2026-07-17; canonical full_name=mem0ai/mem0, archived=false, disabled=false, stars=60997, forks=7100, created_at=2023-06-20, pushed_at=2026-07-16, primary language=TypeScript, license=Apache-2.0, default_branch=main"
  - "[GH:issues] GitHub Search API queried 2026-07-17; open issues=185 and open PRs=439 for repo:mem0ai/mem0"
  - "[GH:languages] https://api.github.com/repos/mem0ai/mem0/languages queried 2026-07-17; TypeScript=3269181, Python=3188333, Shell=123896, JavaScript=38237, CSS=22059; TypeScript only narrowly leads Python, so the repository remains polyglot"
  - "[GH:release] https://api.github.com/repos/mem0ai/mem0/releases/latest queried 2026-07-17; latest GitHub release remains CLI-specific: cli-node-v0.2.11 / Mem0 Node CLI v0.2.11 published 2026-07-13; release body records a dynamic URL-path encoding bug fix"
  ...
  - "[GH:community] https://api.github.com/repos/mem0ai/mem0/community/profile queried 2026-07-17; health_percentage=75; README, CONTRIBUTING, PR template and license detected; code_of_conduct not detected"
  - "[GH:advisories] https://api.github.com/repos/mem0ai/mem0/security-advisories?per_page=100 queried 2026-07-17; returned [] — no repository advisory found in this check, not a safety guarantee"
  - "[GH:readme] https://github.com/mem0ai/mem0/blob/main/README.md fetched through GitHub Contents API 2026-07-17 at blob 9a81e74effacbd7433e91b6c1fcc907f110d4773; distinguishes library, self-hosted server and cloud platform; documents agent signup, skills, auth-on self-hosting and Apache-2.0"
  - "[GH:local-scan] Historical local clone /tmp/repo-wiki-batch3/mem0 at commit 74d043731b9f3ef5d89dcbd435e359b885be5add dated 2026-05-19; git ls-files=2070, Python files=620, TypeScript files=299, TSX files=227, markdown/mdx files=443, workflows=14, test/spec-ish files=246; checked README.md, AGENTS.md, CONTRIBUTING.md, pyproject.toml, server/README.md, cli/node/package.json, docs/, mem0/, mem0-ts/, openmemory/, server/, skills/; retained only for structural claims, not current counts"
  - "[Docs:intro] https://docs.mem0.ai and current README checked 2026-07-17; position Mem0 as a memory layer spanning platform, open source, integrations, API, CLI and agent skills"
  - "[Docs:oss] https://docs.mem0.ai/open-source/overview extracted 2026-07-17; library defaults include OpenAI gpt-5-mini, text-embedding-3-small, local Qdrant and SQLite history; self-hosted server defaults include Postgres/pgvector and bundled OpenAI/Anthropic/Gemini providers"
  - "[Docs:llms] https://docs.mem0.ai/components/llms/overview extracted 2026-07-17; documents OpenAI, Ollama, Azure OpenAI, Anthropic, Together, Groq, LiteLLM, Mistral, Google AI, Bedrock, DeepSeek, MiniMax, xAI, Sarvam AI, LM Studio and LangChain"
  - "[Docs:vectordbs] https://docs.mem0.ai/components/vectordbs/overview extracted 2026-07-17; documents broad Python vector-store support and a narrower explicit TypeScript backend list, including Qdrant, Redis, PGVector, Supabase, LangChain, Azure AI Search, Vectorize, S3 Vectors, Milvus, Neptune Analytics and in-memory"
  - "[Docs:cli] https://docs.mem0.ai/platform/cli extracted 2026-07-17; npm/Python CLIs expose matching add/search/list/get/update/delete behavior and agent-oriented structured JSON via --agent/--json"
  - "[Docs:server] https://docs.mem0.ai/open-source/setup extracted with source-fidelity fetch 2026-07-17; reference stack documents Docker/Compose, REST API + dashboard, JWT/per-user API keys, Postgres/pgvector, browser-first and agent-first bootstrap, live request audit log and AUTH_DISABLED/legacy ADMIN_API_KEY migration cautions"
  - "[PyPI] PyPI JSON queried 2026-07-17; mem0ai=2.0.12 requires_python>=3.10,<4.0 and mem0-cli=0.2.10 requires_python>=3.10"
  - "[NPM] npm view queried 2026-07-17; mem0ai=3.1.0 Apache-2.0 Node>=18, @mem0/cli=0.2.11 Apache-2.0 Node>=18, @mem0/vercel-ai-provider=3.0.0 Apache-2.0 Node>=18"
---
...
> **核验版本**: GitHub API / README / Docs / npm / PyPI 快照 2026-07-17；结构性 local scan 仍为 2026-05 commit `74d0437`
...
推荐度 3/5。加分理由：Mem0 的能力面和扩展面在 memory infrastructure 中非常完整，支持 Python/TypeScript、CLI、server、dashboard、平台 API、多 LLM 与多 vector database；社区活跃度也高 [Docs:llms][Docs:vectordbs][Docs:cli][GH:api]。维持 3 而非 4 的理由：memory layer 牵涉隐私、删除/更新语义、数据保留、auth、audit logs 与成本；同时项目仍高速迭代，open PR 数较高，生产采用前需要实测 retrieval quality、数据治理和运维复杂度 [GH:issues][Docs:server]。
...
4. **自托管路径存在**：当前 reference stack 提供 REST API、dashboard、JWT/per-user API keys、`make bootstrap`、Postgres/pgvector、request audit log 与迁移说明 [Docs:server]。
5. **社区与维护活跃**：GitHub API 快照显示 60k+ stars、7.1k forks，仓库在 2026-07-16 仍有 push；但 185 个 open issues 与 439 个 open PRs 也代表显著维护压力 [GH:api][GH:issues]。

## 劣势

1. **系统面重**：真正生产部署不是 `pip install` 就结束；向 reference self-hosted server 走会涉及 REST API、dashboard、Postgres/pgvector、JWT/API keys、audit logs、模型/embedding provider 与数据库迁移 [Docs:server][Docs:oss]。
...
4. **open PR backlog 较大**：本次 API 快照显示 open PRs=439，且高于 open issues=185；这既反映生态活跃，也说明 review/合并压力很高 [GH:issues]。
...
| Self-hosted server | 中到高 | 中到高 | 随数据增长 | REST API + dashboard + Postgres/pgvector；request audit log、auth state 与数据库迁移都需运维 [Docs:server] |
...
- **外部依赖**：LLM provider、embedding provider、vector DB；graph-memory 配置还会引入 graph store，reference server 另依赖 Postgres/pgvector 与 dashboard [Docs:server][Docs:llms][Docs:vectordbs]。
...
扣一分是因为真实上手很快进入配置分叉：cloud vs self-hosted、Python vs Node、LLM/embedding provider、vector DB、graph memory、API key、auth、dashboard 与 audit-log retention。对简单 demo 来说顺滑；对生产系统来说需要架构选择。

## 代码质量

评分 4/5。

2026-05 的历史本地扫描显示 Mem0 是大型 polyglot monorepo：2070 个 tracked files，Python/TypeScript/MDX 为主，14 个 GitHub workflows，约 246 个 test/spec-ish 文件；AGENTS.md 明确拆分 mem0 Python SDK、mem0-ts、CLI、Vercel provider、OpenClaw、server、openmemory、skills、docs、tests、evaluation 等模块，并给出 build/test/lint 命令 [GH:local-scan]。2026-07 languages API 中 TypeScript 已以 3.27M bytes 略高于 Python 的 3.19M，因此 frontmatter 主语言改为 TypeScript；这只是 GitHub 当前主语言标签变化，不意味着 Python SDK 退居次要 [GH:languages]。
...
| 维度 | 评分 | 说明 |
|------|------|------|
| 社区活跃度 | 4/5 | 60k+ stars、7.1k forks、近期仍有 push，community health=75%；但 open PRs=439、未检测到 CODE_OF_CONDUCT，治理成熟度未到满分 [GH:api][GH:issues][GH:community] |
...
- **API key 与日志**：server 提供 per-user keys、revocation 与每次 API call 的 audit log；日志保留策略、访问控制和敏感字段处理需由部署者明确 [Docs:server]。
- **认证迁移边界**：`AUTH_DISABLED=true` 仅供本地开发；legacy `ADMIN_API_KEY` 为兼容路径，团队应优先采用 JWT 与 per-user keys，并确保 `JWT_SECRET` 强且稳定 [Docs:server]。
