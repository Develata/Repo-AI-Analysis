...
evidence: "current GitHub API/release/advisory metadata + current README + npm registry metadata + current issue sample + prior repository scan; not deployed in production by Develata"
archived_reason: ""
...
estimated_storage: "oh-my-openagent npm unpacked package about 34.9 MB and lazycodex-ai about 19.8 MB for v4.17.0; local state/config/worktrees are project-dependent"
status: active
ratings:
  capability: 4
  usability: 3
...
sources:
  ...
  - "[GH:api] https://api.github.com/repos/code-yeongyu/oh-my-openagent checked 2026-07-13 UTC+8; archived=false, default_branch=dev, language=TypeScript, license=NOASSERTION, stars=65615, forks=5348, open issues=391, open PRs=494"
  - "[GH:languages] https://api.github.com/repos/code-yeongyu/oh-my-openagent/languages queried 2026-05-18"
  - "[GH:releases] https://github.com/code-yeongyu/oh-my-openagent/releases/tag/v4.17.0 published 2026-07-11; first-class LazyCodex/Codex routing and ULW recovery/evidence hardening"
  - "[GH:issues] GitHub GraphQL checked 2026-07-13 UTC+8; open issues=391, open PRs=494; sampled #6061 stale embedded version in v4.17.0 packages, #6009 GPT-5.6 worker skip, #4689 background workers timing out, #5569 parent wake failure"
  - "[GH:prs] https://github.com/code-yeongyu/oh-my-openagent/pulls checked via GraphQL 2026-07-13 UTC+8; open PRs=494"
  ...
  - "[GH:advisories] https://api.github.com/repos/code-yeongyu/oh-my-openagent/security-advisories?per_page=100 checked 2026-07-13 UTC+8; returned [] (no published repository advisory found in this check, not proof of safety)"
  ...
  - "[GH:readme] https://github.com/code-yeongyu/oh-my-openagent/blob/dev/README.md checked 2026-07-13 UTC+8; documents Ultimate/OpenCode and Light/Codex editions plus multi-harness refactor"
  ...
  - "[NPM:openagent] https://registry.npmjs.org/oh-my-openagent/latest checked 2026-07-13 UTC+8; latest=4.17.0, license=SUL-1.0, unpackedSize=34882490"
  - "[NPM:opencode] https://registry.npmjs.org/oh-my-opencode/latest checked 2026-07-13 UTC+8; latest=4.17.0, license=SUL-1.0, unpackedSize=34882477"
  - "[NPM:lazycodex] https://registry.npmjs.org/lazycodex-ai/latest checked 2026-07-13 UTC+8; latest=4.17.0, license=SUL-1.0, unpackedSize=19832341"
---
...
> **核验版本**: GitHub / npm `v4.17.0` 快照 2026-07-13；prior local scan 仍固定在 2026-05 dev commit
...
但它的风险也来自同一结构。安装文档默认建议“让 LLM agent 安装”，并要求根据 Claude/OpenAI/Gemini/Copilot/Z.ai/Kimi/Vercel 等订阅组合生成 flags；匿名 telemetry 默认开启；包名已从 `oh-my-opencode` 迁移到 `oh-my-openagent`，2026-07 的 README 又进一步拆出 Ultimate/OpenCode 与 Light/Codex（`lazycodex-ai`）两条 edition；GitHub API 快照显示 391 open issues、494 open PRs，近期仍有 embedded version、GPT worker skip、background worker timeout、parent wake failure 等兼容性问题；repo 未见 `SECURITY.md`，但产品会运行 hooks、MCP、shell-like tools 与外部 provider 配置 [GH:readme][Docs:installation][Docs:privacy][GH:issues][GH:prs][GH:local-scan]。
...
给 3 而不是 4 的理由很明确：extensibility 仍接近顶格，但 capability 需要因 edition/refactor churn 与当前 backlog 保守折价；maturity 和 security 明显不能顶格。项目创建于 2025-12，默认分支为 `dev`，2026-07 最新 `v4.17.0` 已引入 LazyCodex / Codex routing 等变化，npm `oh-my-openagent`、`oh-my-opencode` 与 `lazycodex-ai` 三个包同时存在；当前 open issues / PRs 已分别到 391 / 494，说明稳定性和合并队列压力都很大 [GH:api][GH:releases][NPM:opencode][NPM:openagent][NPM:lazycodex][GH:issues]。
...
1. **能力密度很高**：多 agent、model routing、background agents、Team Mode、LSP/AST-grep、built-in MCPs、hash-anchored edit、todo enforcer、tmux integration 等被统一放进一个 OpenCode plugin/runtime 体系；v4.17 又把 Codex/LazyCodex 路线抬成更明确的 edition 分支 [Docs:overview][Docs:orchestration][Docs:team-mode][GH:readme][GH:releases]。
...
5. **可见社区动能极强**：2026-07-13 GitHub API 显示 65,615 stars、5,348 forks；2026-05 contributors endpoint 曾显示约 233 contributor pages。可见热度极高，但半年内极高 star 增长应折价理解为“可见热度”而非质量证明 [GH:api][GH:contributors]。
...
2. **稳定性受快速迭代限制**：npm 版本密集、edition/brand/refactor churn、391 open issues 与 494 open PRs 表明接口和行为仍在高速变化；v4.17.0 自身也有 stale embedded version 等问题报告 [GH:releases][NPM:opencode][NPM:openagent][NPM:lazycodex][GH:issues]。
...
capability 评分 4/5。理由是它覆盖了 OpenCode / Codex 复合型 AI plugin 的多数重要维度，但当前 edition 拆分、rename/refactor churn 和较高 bug backlog 让“能力广度”不能直接等同于稳定可用覆盖：
...
这里的 4 是“能力广度很强但稳定覆盖不足”。若未来 OpenCode 与 LazyCodex 两条路线形成清晰边界、稳定 release cadence 和更低 bug backlog，capability 才适合重新上调到 5；当前不能把 README 能力声明当作成熟覆盖。

## 运行环境与资源占用

| 场景 | CPU | 内存 | 存储 | 说明 |
|------|-----|------|------|------|
| 最小 | 单核足够 | 低到中等 | `oh-my-openagent@4.17.0` unpacked about 34.9 MB；`lazycodex-ai@4.17.0` about 19.8 MB | 仅 plugin/CLI 与单个 OpenCode / Codex session；实际还需要 host 与 provider auth [NPM:openagent][NPM:lazycodex][Docs:installation] |
...
| 维度 | 评分 | 说明 |
|------|------|------|
| 社区活跃度 | 5/5 | 2026-07-13 GitHub API 显示 65,615 stars、5,348 forks；2026-05 contributors endpoint 曾显示约 233 contributor pages。可见热度极高，但半年内极高 star 增长应折价理解为“可见热度”而非质量证明 [GH:api][GH:contributors] |
| 成熟度 | 2/5 | 创建于 2025-12，默认分支为 `dev`，v4.17.0 仍在快速迭代；391 open issues、494 open PRs、多个 npm 包/edition 并存、rename/refactor transition 与 worker/runtime bug 均显示 API/行为稳定性不足 [GH:api][GH:releases][GH:issues][GH:prs][NPM:opencode][NPM:openagent][NPM:lazycodex] |
...
- GitHub API、GitHub GraphQL、GitHub advisory API 与 npm registry 快照更新到 2026-07-13；prior local clone/code scan 仍为 2026-05-18。
