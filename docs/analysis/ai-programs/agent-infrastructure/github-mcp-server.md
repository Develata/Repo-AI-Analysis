...
tags: ["agent-infrastructure", "mcp", "github", "github-api", "coding-agents", "copilot", "tooling", "go", "oauth", "pat", "docker"]
previous_repo: ""
successor: ""
primary_language: "Go"
license: "MIT"
...
是。GitHub MCP Server 很适合写成 **“agent 的权限边界不是 prompt，而是 token scope 和 tool surface”** 的 sharing。它一方面是值得参考的官方 MCP connector 样本，另一方面也提醒：当 agent 能直接操作 GitHub，工程治理就不能停留在“相信模型会小心”，而要落实到 token scope、tool surface、审计与人工确认。
...
对 Develata 的 wiki 来说，它尤其适合作为“agent-infrastructure 不只是能力，更是权限边界”的案例。给 agent 工具，实即给 agent 可能性；给 agent token，实即给 agent 权力。权不可以无名，器不可以无制。