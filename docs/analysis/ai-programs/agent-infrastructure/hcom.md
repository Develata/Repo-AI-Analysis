...
sources:
  ...
  - "[WikiLocal:comparisons] Local github-repo-wiki entries inspected 2026-06-24 for adjacent agent-infrastructure / ai-harness positioning: vibe-kanban.md, oh-my-claudecode.md, oh-my-openagent.md, cli-anything.md"
---
...
hcom 适合多个 terminal agent 需要长期并发、彼此通信、观察、唤醒和管理生命周期的场景；如果只是 Claude/Hermes 调 Codex CLI 做一次 review 或 second opinion，skill + CLI 的单次主从调用更简单、更稳，也更符合最小复杂度原则。
...
因此我把它归入 `ai-programs/agent-infrastructure`，而不是 `coding-agents`。它更像 terminal agent mesh 的 runtime glue：提供 agent identity、inbox、transcript、terminal screen、event log、collision notification、TUI、workflow scripts、cross-device relay。这个方向对多代理工作流很有学习价值，但它的主场不是“调用另一个模型/CLI 做一次任务”，而是“多个活着的 agent session 如何自治通信、互相观察，并在合适时机交换上下文”。在 Hermes 或 Claude 作为唯一 controller 的普通 review flow 里，直接 skill + CLI 调 Codex 通常更优。

分数保持克制：能力、体验、代码质量、文档和扩展性都可以给 4；但社区规模仍小、项目仍是 Beta/0.7.x，且安全模型天然高权限，所以成熟度和安全不宜高估 [GH:api][GH:graphql][PyPI][GH:issues]。

## 推荐度：4/5

**定位**：推荐给已经熟悉 Claude Code / Codex / Gemini CLI / OpenCode 等 terminal agent，并且确实需要多个 agent session 同时存活、互相发消息、互相 watch、互相 fork/resume/kill 的高级个人开发者、AI workflow tinkerer 和 agent-infrastructure 研究者。
...
作为“值得试用/跟踪”的工具，它比普通 prompt/skill 仓库更接近可运行 infrastructure；但对于 Claude 调 Codex review、Hermes 调 GPT-5.5 blind review、单次问答/审查这类主从式调用，hcom 属于过度架构，skill + CLI 更清晰。作为“团队生产依赖”，还需要 pin 版本、隔离 `HCOM_DIR`、限制 relay trust domain，并先用非关键 repo 做小规模演练。
...
6. **单次 reviewer 调用不需要它**：如果工作流是“Claude/Hermes 构造 prompt → 调 Codex CLI → 读回 review result”，hcom 的 message bus、TUI、hook、relay 和 session identity 都不会带来核心收益，反而增加调试和权限面。

---

## 适合什么场景

- 同时跑 Claude Code、Codex、Gemini CLI、OpenCode 等多个 terminal agent，而且这些 agent 不是一次性子进程，而是需要长期存活、互相发消息、互相等待、互相观察进度 [GH:local-scan][Local:cli]。
...
- Claude / Hermes 作为唯一 controller，只是调用 Codex CLI、Gemini CLI 或 Reasonix 做一次 review / second opinion / blind review 的场景；这种主从式 tool call 应优先使用 skill + CLI、prompt/result 文件和现有 delegation。
...
上述项目跨 `ai-programs/agent-infrastructure` 与 `ai-programs/ai-harness/ai-plugins` 做 adjacent positioning，对比基于本地 wiki 既有条目的标题级定位与分类语境，未按同一 10 维度框架重新深审 [WikiLocal:comparisons]。
...
- 边界上，它不替代 skill + CLI / one-shot reviewer 调用；`codex exec` 这类主从式审查仍应直接用 CLI，hcom 只在 Codex/Claude/Gemini 等也需要作为长期 session 互相通信、等待、观察或被唤醒时才提供额外价值。
...
扣 1 分的原因是它的价值不是“装完即懂”：用户需要已经理解各 agent CLI、hook 注入、terminal/pane、消息投递、relay token、`HCOM_DIR` 隔离等概念。即使已经有 Hermes/Codex/Claude 多代理使用场景，也要先判断自己需要的是 **tool call orchestration** 还是 **agent-to-agent coordination**：前者用 skill + CLI 更顺，后者才是 hcom 的空间。
...
hcom 的学习价值很高，因为它提出了一个朴素但关键的判断：terminal agent 生态不会只需要“更强的单 agent”，还可能需要一个 agent 间通信与观察协议层。它没有做厚重 web UI，也没有重新发明 coding agent，而是把 hooks、SQLite、PTY、TUI、relay 和 workflow scripts 组合成一个轻量 coordination substrate。

对 Develata 来说，它值得重点研究三点：第一，mid-turn / idle-wake message delivery 如何改变多 agent 协作；第二，local event log + transcript + terminal screen 如何形成可查询 agent state；第三，relay security model 如何在个人设备 trust domain 中取舍简单性与权限风险。与此同时，它也给出一个反面边界：能用 skill + CLI 单次调用解决的 review/问答/审查，不应为了“多 agent 感”引入 hcom。其可取处在“少而锐”，其可畏处亦在“通而高权”；善用则为器，不审则为患。