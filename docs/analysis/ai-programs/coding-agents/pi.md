...
evidence: "official GitHub API/release/advisory metadata + current README/docs + npm registry + prior code review; not newly smoke-tested"
archived_reason: ""
successor: ""
...
sources:
  ...
  - "[GH:API] https://api.github.com/repos/earendil-works/pi checked 2026-07-13 UTC+8; archived=false, default_branch=main, language=TypeScript, license=MIT, stars=69893, forks=8613, open issues=55, open PRs=11"
  - "[GH:release] https://github.com/earendil-works/pi/releases/tag/v0.80.6 published 2026-07-09"
  - "[GH:advisories] https://api.github.com/repos/earendil-works/pi/security-advisories checked 2026-07-13 UTC+8; four published advisories: GHSA-7v5m-pr3q-6453 (Low), GHSA-r95r-rj6r-c39x (Low), GHSA-jfgx-wxx8-mp94 (High), GHSA-mqxh-6gq7-558m (Moderate)"
  - "[GHSA:pi] https://github.com/earendil-works/pi/security/advisories; @earendil-works/pi-coding-agent fixes: 0.78.1 for GHSA-7v5m/GHSA-r95r/GHSA-jfgx, 0.79.0 for GHSA-mqxh; checked 2026-07-13 UTC+8"
  - "[NPM] https://registry.npmjs.org/@earendil-works/pi-coding-agent/latest checked 2026-07-13 UTC+8; version=0.80.6, node>=22.19.0, unpackedSize=12498235"
...
Pi 是个奇怪的近 70k star 项目：它的核心只有 4 个工具（read/write/edit/bash），却通过 TypeScript 扩展系统、15+ 模型提供商支持、树状会话管理和四种运行模式（交互/TUI、print/JSON、RPC、SDK），把「终端编码代理」这件事做到了极致的可组合性 [GH:API]。
...
痛点在于成熟度与安全边界：项目仍是 v0.x，当前审计看到最新 release 为 `v0.80.6`，相邻 release 仍可能改变扩展 API、模型配置或运行行为；同时 2026-07 的 GitHub advisory 检查已经能看到 4 条公开 advisory，其中包含 High / Moderate 项，虽已在 `0.78.1` / `0.79.0` 后修复，但足以说明终端 agent + 扩展系统不能再按“无已知 CVE”来高估安全分 [GH:release] [GH:advisories] [GHSA:pi]。这种高速迭代对尝鲜者是兴奋剂，对生产团队是风险源。好在 MIT 许可证和单体维护者（Mario Zechner，libgdx 作者）意味着即使项目停滞，fork 路径畅通。
...
7. **MIT 许可证**：无商业使用顾虑，可 fork 可商用；截至 2026-07-13，GitHub API 显示 69,893 stars / 8,613 forks，社区可见度极高 [GH:API]。
...
负面信号：单体维护者意味着代码审查是瓶颈。auto-close 策略虽然过滤了低质量贡献，但也减少了来自社区的质量信号输入。55 个开放 issue 对近 70k star 项目来说极低，但这可能仅仅是筛选门槛高而非代码质量好到无需修复 [GH:API]。
...
| 维度 | 评分 | 说明 |
|------|------|------|
| 社区活跃度 | 4/5 | 2026-07-13 GitHub API 显示 69,893 stars、8,613 forks。Discord / GitHub 可见热度无可争议，但单体维护者模型（Mario Zechner 基本单人主导）和贡献者准入制度（auto-close + 人工筛选）限制了「真正的社区驱动」定义 [GH:API] |
| 成熟度 | 2/5 | 项目仍处 v0.x；当前 npm / release 快照为 `0.80.6`，且近期仍能看到与扩展 API、模型配置和 provider 行为相关的快速演进。它符合「频繁破坏性变更，无稳定 API 承诺」得分边界 [GH:release] [NPM] |
...
**公开 advisory**：本次 GitHub repository security advisory 检查不再为空，记录到 4 条 advisory：两条 Low、一条 High、一条 Moderate；API 元数据记录 `@earendil-works/pi-coding-agent` 已在 `0.78.1` / `0.79.0` 修复这些问题。这里不能把 advisory 数量等同于当前最新版仍易受攻击，但它们改变了安全评分的证据基础 [GH:advisories] [GHSA:pi]。
...
安全评级 3/5：Pi 在编码代理中仍属于边界设计较克制的一类，但终端 agent、bash 工具、完整权限扩展系统和近期公开 advisory 共同决定它不能给到 4。采用时应固定版本、只安装可信扩展、关注 GitHub advisory，并把扩展当作本机高权限代码执行面。
