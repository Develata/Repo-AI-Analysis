...
n8n 的主分类在本次 taxonomy 中改为 `ai-programs/ai-workflow`：它原本是 workflow automation / integration platform，但在 2026 的语境下，AI nodes、LangChain nodes、agent、memory、human-in-the-loop 已经成为其重要能力面，适合放入 AI Programs 下的 AI Workflow。

## 总体评价

n8n 是一个大型、成熟、生态极强的 workflow automation 平台。它的核心不是“单个 agent”或“agent infrastructure primitive”，而是把触发器、节点、凭据、数据转换、外部 API、数据库、队列、人工审批、日志和部署治理组织成可视化 workflow [GH][Docs:home]。因此本次归入 `ai-programs/ai-workflow`，而不是 `ai-programs/agent-infrastructure`。
...
5. **AI workflow 融合自然**：LangChain nodes 可配置 agents、LLMs、memory，并和普通 n8n nodes 连接，适合把 AI 作为 workflow 中的一等节点 [Docs:ai]。
...
- 团队希望用可视化 workflow 降低非纯工程成员接入自动化/AI workflow 的门槛。
...
- 想研究 workflow engine、node/plugin architecture、low-code automation、AI workflow 平台化。
...
上述项目按 `ai-programs/ai-workflow` 同类范围做定位级对比，未按同一 10 维度框架深审。n8n 与 Dify、Flowise 这类 LLM / agent application platform 有交叉；差别在于 n8n 的底色仍是通用 automation，而 Dify/Flowise 更贴近 AI application / agent workflow builder。

## 个人主页 sharing 候选

是。n8n 适合作为 sharing 候选，但建议角度不是“自动化神器”这种泛介绍，而是写 **workflow automation 如何吸收 agent capabilities**：从 Zapier/IFTTT 式自动化，到 LangChain nodes、MCP、Agent 365、human-in-the-loop，再到安全/凭据/SSRF/自托管治理的代价。
...
- **Advanced AI**：Cloud/self-hosted 中的 AI workflows、LangChain nodes、agents、LLMs、memory、Chat Trigger、chatbot widget [Docs:ai]。
