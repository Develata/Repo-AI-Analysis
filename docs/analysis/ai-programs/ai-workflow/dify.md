...
Dify 更准确地说是低代码 agent / LLM application **platform**，不是传统 framework/library；在当前 taxonomy 下归入 `ai-programs/ai-workflow` 更合适，因为它的主对象是可视化 AI workflow、RAG、agent 与应用发布。
...
上述项目是与 Dify 相邻的 AI app / framework / workflow 生态参照，未按同一 10 维度框架深审；其中 LangChain、LangGraph、LlamaIndex、Vercel AI SDK、CrewAI 主要是 code-first framework/runtime，不是 `ai-programs/ai-workflow` 下的直接同类。Dify 与 n8n、Flowise 更适合作为低代码 AI workflow 平台共同比较，而与 LangChain 这类 code-first framework 只构成相邻参照。
