...
OpenFang 应归入 `ai-programs/agent-assistants`，而不是纯 `agent-infrastructure`：它的核心交付对象不是一个被其他 agent framework 嵌入的小库，而是一个用户可安装、可启动 daemon/dashboard/desktop、可激活 Hands 并通过多通道持续工作的自托管 agent system [GH:getting-started][GH:architecture]。
