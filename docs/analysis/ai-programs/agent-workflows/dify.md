...
evidence: "GitHub GraphQL/REST + releases/advisories + official docs + npm metadata + shallow local scan at 40df83d; no fresh deployment, upgrade rehearsal, security test, or production benchmark was performed"
archived_reason: ""
docker_support: true
gpu_required: false
...
overall_score: 3.9
sources:
  - "[GH] https://github.com/langgenius/dify — repository and README verified 2026-07-15"
  - "[GH:api] GitHub GraphQL queried 2026-07-15 — 148824 stars, 23435 forks, 283 open issues, 688 open pull requests, 166 releases; main HEAD 40df83de660d19b17d9674821e82aa2bd4b61a49; GitHub license metadata NOASSERTION"
  - "[GH:issues] GitHub GraphQL queried 2026-07-15 — 283 open issues and 688 open PRs are separate counts"
  - "[GH:languages] GitHub GraphQL queried 2026-07-15 lists TypeScript as primary; local scan confirms a large TypeScript/Python monorepo"
  - "[GH:release] https://github.com/langgenius/dify/releases/tag/1.15.0 checked 2026-07-15 — latest stable 1.15.0 published 2026-06-25 adds difyctl, streamed chain-of-thought views, richer human-input forms, long-running-model polling, observability and security fixes; newer 1.16.0-rc1 is prerelease; stable upgrade requires DB migration, plugin auto-upgrade backfill and env review"
  - "[GH:contributors] GitHub contributors API first page queried 2026-07-15 — 100 entries; top counts laipz8200 708, lyzno1 542, takatost 539, iamjoel 498, JohnJyong 370"
  ...
  - "[GH:advisories] GitHub repository security-advisories API queried 2026-07-15 — 17 published advisories: 5 high, 9 medium, 3 low. Samples include GHSA-qqjx-5h5w-x5vj (high DOM XSS, <=V1.11.4), GHSA-phpv-94hg-fv9g (high plaintext API key exposure, api <=1.10.1-fix.1), and GHSA-8235-vv5j-mmvg (high unauthenticated SSRF, <1.13.0). API returned no first_patched_version for some samples; advisory-level checking is required"
  - "[GHSA:rbac] https://github.com/langgenius/dify/security/advisories/GHSA-hqcx-598m-pjq4 metadata rechecked 2026-07-15 — improper access control allowed a normal authenticated user to enable/disable apps"
  - "[GHSA:mcp-xss] https://github.com/langgenius/dify/security/advisories/GHSA-9jch-j9qf-vqfw metadata rechecked 2026-07-15 — MCP OAuth XSS when connecting to an attacker-controlled remote MCP server"
  - "[GH:local-scan] shallow clone main@40df83de660d19b17d9674821e82aa2bd4b61a49 inspected 2026-07-15 — `git ls-files` returned 13508 tracked paths and 32 paths under `.github/workflows/`; README, AGENTS.md, CONTRIBUTING.md, LICENSE, docker and package manifests inspected"
  - "[Docs:intro] https://docs.dify.ai/ and repository README checked 2026-07-15 — agentic workflow platform, tools/data sources, AI application deployment"
  - "[Docs:llms] https://docs.dify.ai/llms.txt and https://docs.dify.ai/en/cli/overview checked 2026-07-15 — API, plugin, self-hosting, app/knowledge workflows and difyctl scope"
  - "[Docs:docker] https://docs.dify.ai/getting-started/install-self-hosted/docker-compose extracted 2026-07-15 — Docker Compose 2.24.0+, 2 CPU/4 GiB minimum, Docker Desktop guidance includes 8 GiB, and current stack has 6 core services + 6 dependent components + init task"
  - "[GH:readme] current README inspected 2026-07-15 — platform scope, Docker quick start, CPU >=2 cores and RAM >=4 GiB"
  - "[GH:license] current LICENSE inspected 2026-07-15 — modified Apache-2.0-like Dify license adds multi-tenant-service and frontend logo/copyright conditions"
  - "[NPM] npm view dify-client checked 2026-07-15 — version 3.1.0, MIT; SDK license does not replace repository/platform license"
---
...
> **核验版本**: GitHub commit `40df83de660d19b17d9674821e82aa2bd4b61a49`；latest stable release `1.15.0`（`1.16.0-rc1` 为 prerelease）；GitHub API / Docs / npm 快照 2026-07-15 [GH:release]

## 一句话总结

Dify 更准确地说是低代码 agent / LLM application **platform**，不是传统 framework/library；在当前 taxonomy 下归入 `ai-programs/agent-workflows` 更合适，因为它的主对象是可视化 agent workflow、RAG、agent 与应用发布。
...
分类上它应从 `ai-programs/agent-frameworks` 迁入 `ai-programs/agent-workflows`。理由是：`agent-frameworks` 更偏 code-first library/framework，如 LangChain、AutoGen、CrewAI；Dify 则偏平台产品，中心抽象是 workflow/chatflow/app/knowledge base/plugin/publishing，而不是给开发者嵌入代码中的 framework API。它比 `agent-infrastructure` 更高层，比 `assistant-agents` 更底层，也不只是通用 automation。

总体评价：Dify 是功能覆盖极强、社区极热、文档完整的 AI application platform；1.15.0 的 `difyctl`、更丰富的 human-in-the-loop forms、长时模型 polling 与 trace 改进把它进一步推向可操作的 workflow platform [GH:release]。但它也是大而重的多租户 Web 系统，历史安全 advisories 多，license 非标准 Apache，运维/升级/权限治理要求高。它适合认真建设 AI 应用平台的团队，不适合作为“随手跑一个 agent demo”的轻量依赖。
...
5. **社区与采用关注度极高**：API 快照显示 148k+ stars、23k+ forks、contributors 第一页已满 100 人，community profile 达 100 [GH:api][GH:contributors][GH:community]。
...
2. **历史安全问题多**：GitHub API 返回 17 条 security advisories（5 high、9 medium、3 low），涉及 XSS、SSRF、API key exposure、IDOR、access control 等；部分 API metadata 未提供 `first_patched_version`，不能脱离 advisory/实际版本判断 [GH:advisories]。
...
4. **issue/PR backlog 高**：GraphQL 快照显示 open issues=283、open PRs=688；活跃度强，但维护压力也大 [GH:issues]。
5. **升级/配置复杂**：1.15.0 要求数据库迁移、plugin auto-upgrade backfill、19 个新增/2 个移除/1 个修改的环境变量审阅；长期自托管需要严肃管理 `.env`、Compose、数据库迁移和插件版本 [GH:release][Docs:docker]。
...
- 企业内部希望给非纯工程团队一个可视化 agent workflow 平台，而不是要求所有人写 LangChain/LangGraph 代码。
...
上述项目是与 Dify 相邻的 AI app / framework / workflow 生态参照，未按同一 10 维度框架深审；其中 LangChain、LangGraph、LlamaIndex、Vercel AI SDK、CrewAI 主要是 code-first framework/runtime，不是 `ai-programs/agent-workflows` 下的直接同类。Dify 与 n8n、Flowise 更适合作为低代码 agent workflow 平台共同比较，而与 LangChain 这类 code-first framework 只构成相邻参照。
...
- **Workflow / Chatflow / Agent**：可视化构建 agent workflows、chatflows 和 agents，连接工具与数据源 [Docs:intro][Docs:llms]。
...
- **CLI / 人机协同**：`difyctl` 可从 terminal、脚本或 CI 调用 apps/workflows；Human Input 支持 select、file/multi-file upload，长时生成模型可走 polling 等待最终结果 [GH:release][Docs:llms]。
...
| 生产自托管 | 随并发、worker、plugin、RAG 增长 | 通常显著高于最小值 | DB/vector/files/logs 持续增长 | 当前 Compose 启动 6 个 core services、6 个 dependent components 和 1 个 init task [Docs:docker] |
...
本地扫描和 GitHub metadata 显示 Dify 是大型但有组织的 TypeScript/Python monorepo：`git ls-files` 返回 13508 个 tracked paths，其中 32 个位于 `.github/workflows/`；backend API 是 Python，frontend 是 TypeScript，Docker deployment 独立在 `/docker` [GH:languages][GH:local-scan]。
...
社区非常活跃：GitHub API 快照显示 148824 stars、23435 forks；contributors 第一页为 100 人；community profile 100，检测到 README、CONTRIBUTING、CODE_OF_CONDUCT 和 PR template [GH:api][GH:contributors][GH:community]。

community 不给 5 是因为 open issues=283、open PRs=688，维护压力很大；并且 modified license + 商业公司主导平台意味着“社区热”不等于完全开放治理 [GH:issues][GH:license]。

成熟度给 3 是按 stability/change-risk maturity，而不是按 project viability。Dify 已经超过三年、进入 v1.15.0、生产定位明确，但仍在快速演进；本次升级需要 DB migration、plugin backfill 与大量 env 变更，历史 advisories 也说明它还不是“长期稳定、破坏性变更罕见”的成熟平台 [GH:release][GH:advisories][Docs:docker]。

## 安全与风险

评分 2/5。

Dify 的安全面很大：它是多用户 Web 平台，处理登录、workspace/tenant、应用发布、API keys、model provider credentials、file upload、RAG 文档、workflow execution、插件、MCP OAuth、sandbox、ssrf_proxy、数据库和外部工具调用。GitHub Security Advisories API 本次返回 17 条 advisory（5 high、9 medium、3 low）；样本包括高危 DOM XSS、plaintext API key exposure、unauthenticated SSRF、MCP OAuth XSS 与普通认证用户越权启停 app 的 access-control 问题 [GH:advisories][GHSA:mcp-xss][GHSA:rbac]。

这些 advisories 是历史与版本范围证据，不应理解为当前 `1.15.0` 一定仍漏洞暴露；反之，API 对部分样本未给 `first_patched_version`，也不能据此宣称当前版本自动安全。1.15.0 仍修复 plugin-daemon forwarding path traversal 并升级依赖，说明安全维护持续进行 [GH:release][GH:advisories]。
