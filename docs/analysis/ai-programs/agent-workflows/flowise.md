...
`FlowiseAI/Flowise` 应归入 `ai-programs/agent-workflows`，更精确地说是 **visual agent builder** 或 **low-code agent workflow platform**。它不是 CrewAI 那种 Python role-based framework，也不是 AutoGen 那种 multi-agent runtime；它的主张是“Build AI Agents, Visually” [GH:api][GH:readme]。
