...
evidence: "current GitHub API/releases/advisories + official docs + PyPI metadata + prior local clone scan"
archived_reason: ""
previous_repo: ""
successor: ""
docker_support: true
...
sources:
  ...
  - "[GH:api] https://api.github.com/repos/open-webui/open-webui checked 2026-07-13 UTC+8; archived=false, default_branch=main, language=Python, license=NOASSERTION, stars=145150, forks=21034, open issues=242, open PRs=169"
  - "[GH:releases] https://github.com/open-webui/open-webui/releases/tag/v0.10.2 published 2026-07-01"
  - "[GH:issues] GitHub GraphQL current open-issue sample checked 2026-07-13 UTC+8; examples include #27017 structured output persistence, #26860 MCP tool dispatch hang, #27014 mobile sidebar pagination"
  - "[GH:advisories] https://api.github.com/repos/open-webui/open-webui/security-advisories checked 2026-07-13 UTC+8; 100 records returned (endpoint page cap), with 26 advisories published after 2026-05-18 in this check; v0.9.6 fixes the sampled <=0.9.5 wave and v0.10.0 fixes the sampled <0.10.0 wave"
  - "[GH:advisories-current] https://github.com/open-webui/open-webui/security/advisories; sampled post-check High issues include GHSA-frvj-c5qp-xj4w terminal-proxy traversal, GHSA-74h3-cxq7-vc5q cross-user code/tool execution, GHSA-855v-hq7w-jmjw revoked-JWT acceptance, GHSA-j657-m4c4-24jq terminal identity spoofing, GHSA-4r2p-27mh-5m22 shared-chat RCE"
  ...
  - "[PyPI] https://pypi.org/pypi/open-webui/json checked 2026-07-13 UTC+8; version=0.10.2, Python>=3.11,<3.13, wheel=152030106 bytes"
---
...
它的优势很鲜明：功能覆盖广、文档体系完整、部署方式成熟、社区规模巨大。截至 2026-07-13，GitHub API 显示 145,150 stars、21,034 forks、242 open issues、169 open PRs；release 频率仍高，最新稳定 tag 为 `v0.10.2` [GH:api][GH:releases]。

但它不是低风险软件。其一，Open WebUI 是服务器端 Web 应用，天然承载账号、上传文件、聊天历史、RAG 索引、外部 API key 与插件执行能力。其二，本次 GitHub Security Advisories API 检查返回了 100 条记录（达到分页上限），其中 2026-05-18 之后新增 26 条可见 advisory；样本包括路径穿越、跨用户 tool/code execution、revoked JWT 仍被接受、terminal identity spoofing、shared-chat RCE 等 High 级问题 [GH:advisories] [GH:advisories-current]。其三，插件系统明示会在服务器执行任意 Python 代码，这是功能设计，不是偶然实现细节 [Docs:plugins][Docs:cve-0766]。

结论：**个人和小团队自托管 AI 入口，仍值得在受控环境试用；企业/公网/多租户部署，必须按高风险服务器产品来 harden、升级、隔离和审计。**

## 推荐度：3/5

**定位**：适合需要一个自托管、多用户、多模型、多知识库 AI Web 平台的个人、实验室、小团队或组织。

推荐度降为 3，而不是 4，理由是它的 product capability 已接近同类顶层，但 2026-07 的 advisory 密度和权限边界问题已经明显提高了采用门槛：

1. **推荐用于受控环境**：内网、家庭服务器、实验室、教学团队、本地 Ollama 前端、多 provider chat hub，这些场景中 Open WebUI 的收益非常直接。
...
所以推荐语是：**可部署、可研究，但默认只适合受控环境；不要未加固直接公网暴露，更不要把“能 docker run”误读成“已生产安全”。**
...
5. **社区势能巨大**：145k+ stars、21k+ forks、持续 release 与 issue/PR 流动，说明它已经进入事实标准候选 [GH:api]。
...
2. **近期 advisory 很多**：2026-07 GitHub advisory API 已达到 100 条分页上限，且 2026-05-18 之后新增 26 条可见 advisory；样本包含 RCE、权限绕过、路径穿越、JWT/terminal 边界等问题 [GH:advisories][GH:advisories-current]。
...
代码质量不是“差”，而是“工程化成熟但质量压力极大”：结构成熟、自动化存在；但 advisory 密度、disabled CI、宽功能面和快速 release 共同把代码质量评分压到 2/5。这里的 2 是对当前安全/权限缺陷密度和验证压力的保守折价，不是说项目没有工程能力。
...
| 维度 | 评分 | 说明 |
|------|------|------|
| 社区活跃度 | 5/5 | 2026-07-13 GitHub API 显示 145,150 stars、21,034 forks、242 open issues、169 open PRs；社区规模和可见采用仍是同类顶层 [GH:api] |
| 成熟度 | 3/5 | 2023-10 创建，v0.10.2 最新 release，迭代速度快但仍未到 1.0；功能面和部署实践成熟，API/安全面仍在快速变化 [GH:api][GH:releases] |
