...
因此我把它放在 `agent-programs/agent-assistants`，并明确状态为 `active-platform-classic-deprecated`：这是按 Classic/agent-history lineage 归类，不表示当前 AutoGPT Platform 只是简单 assistant；当前 product surface 已明显接近 workflow automation / agent platform。
...
上述项目按 `agent-programs/agent-assistants` 同类/相邻范围做定位级对比，未按同一 10 维度框架深审；这里主要用于 agent 史定位。
