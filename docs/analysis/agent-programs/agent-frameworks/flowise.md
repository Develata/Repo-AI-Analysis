...
tags: ["visual-agent-builder", "low-code", "agentflow", "rag", "typescript", "langchain", "self-host", "mcp"]
overall_score: 3.7
recommendation: "适合作为 visual agent builder / low-code LLM app platform 收录；生产采用需重点审查许可证、RBAC/tenant isolation、credential handling 与历史 advisories。"
status: active
...
`FlowiseAI/Flowise` 应补进 `agent-programs/agent-frameworks`，更精确地说是 **visual agent builder** 或 **low-code agent workflow platform**。它不是 CrewAI 那种 Python role-based framework，也不是 AutoGen 那种 multi-agent runtime；它的主张是“Build AI Agents, Visually” [GH:api][GH:readme]。
...
| 创新性 | 4/5 | visual agent workflow vocabulary 清晰 [GH:agentflow][Docs:agentflow] |
