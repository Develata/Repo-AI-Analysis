...
它应该归入 `agent-programs/coding-agents`，但它的历史意义比当前实用推荐更突出。Plandex 很早就明确了几条后来 coding agents 都在追的设计线：
...
这里按 `agent-programs/coding-agents` 同类/相邻项目做定位级对比；部分项目能力随时间快速变化，未在本表做同一 10 维度深审，比较结论仅用于 positioning。
