...
从定位上看，Tabby 与 Cline / Codex / Claude Code 的差别很大。它不是以“agent 自主改代码、执行命令、跑浏览器”为核心，而是以自托管代码补全、chat、模型服务、IDE extension、代码索引和企业部署为核心。它应归入 `agent-programs/coding-agents` 是因为它面向 coding AI assistant 主线；但正文需明确：它是 assistant / infrastructure hybrid，而非强 autonomous agent。
...
上述项目按 `agent-programs/coding-agents` 同类范围做定位级对比，未按同一 10 维度框架深审。Tabby 是边界案例：也可被理解为 `agent-programs/agent-infrastructure` 下的 coding-assistant backend，但为便于与 Continue/Cline/Codex 对比，暂归 coding-agents。

## 个人主页 sharing 候选

是，但主题不应写成“Tabby 是最强 coding agent”。更合适的题目是：**“Self-hosted coding assistant 的工程边界：Tabby 为什么更像基础设施而不是 agent？”**

它适合用来讨论代码隐私、本地模型、GPU 成本、模型 license、IDE extension、OpenAPI、企业版许可证和 autonomous agent 的边界。
...
3：覆盖 self-hosted code completion/chat/IDE/API/model registry 主场景，但在 coding-agent rubric 下缺少强 autonomous execution 能力。

### usability（上手体验）

4：Docker/文档路径清楚，但硬件和模型选择增加复杂度。

### performance（资源效率）

3：资源效率取决于模型；CPU-only 不理想，GPU 推荐。

### code_quality（代码质量）

4：Rust workspace 分层较清楚，有测试说明；未跑完整验证。

### documentation（文档质量）

4：主文档和模型 registry 清楚，但路径迁移/旧 URL 需注意。
...
3：0.x，alpha/nightly 并存。

### extensibility（可扩展性）

4：OpenAPI、IDE extension、模型后端和 crate 分层提供扩展面。

### security（安全性）

3：无 repo advisory 返回，但存在部署、模型、license、API 暴露等常规风险。

### recommendation（推荐度）

3：适合 self-hosted coding assistant 场景；若目标是 autonomous coding agent，优先级较低。
