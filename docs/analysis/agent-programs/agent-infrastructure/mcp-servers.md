...
因此，这个仓库应归入 `agent-programs/agent-infrastructure`，而不是普通 dev-tools。它解决的是 agent 如何通过标准协议访问 filesystem、git、memory、time、fetch 等外部能力；这是 tool/data access substrate，而不是某个最终用户应用 [Docs:mcp-intro][Docs:readme]。
