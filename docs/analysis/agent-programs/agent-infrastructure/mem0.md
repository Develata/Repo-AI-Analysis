...
tags: ["agent-infrastructure", "memory", "long-term-memory", "agents", "rag", "python", "typescript", "cli", "self-hosted", "mcp"]
previous_repo: ""
successor: ""
...
Mem0 应归入 `agent-programs/agent-infrastructure`。它不是一个终端 coding agent，也不是普通 RAG demo，而是试图成为 agent 应用的 **memory substrate**：Python/TypeScript SDK、hosted platform、自托管 server、CLI、agent skills、Vercel AI SDK provider、OpenMemory dashboard、vector stores、graph/entity linking 与多 LLM/embedding provider 共同组成了完整 memory stack [GH][Docs:intro][Docs:oss][GH:local-scan]。
