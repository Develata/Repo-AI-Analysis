...
上述项目按 `agent-programs/agent-infrastructure` 同类范围做定位级对比，未按同一 10 维度框架深审。cal.diy 是该目录中的边界型项目：它不是 agent-specific，但可作为 agent scheduling infrastructure 使用；若后续 wiki 新增 `scheduling/` 或 `self-hosted-saas/` 目录，可考虑迁移。
