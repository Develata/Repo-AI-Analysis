...
evidence: "GitHub REST/GraphQL metadata, official docs, release notes and local shallow clone/source scan at commit be3c083ac7875db9a1919e9bc155e033fadab999; Docker, desktop app, MCP auth and presentation export were not smoke-tested"
archived_reason: ""
docker_support: true
gpu_required: false
...
sources:
  ...
  - "[GH:api] GitHub REST/GraphQL snapshot 2026-07-21: canonical repo=presenton/presenton, archived=false, disabled=false, default_branch=main, head=be3c083ac7875db9a1919e9bc155e033fadab999, pushed_at=2026-07-20T18:08:05Z, stars=9122, forks=1427, open issues=22, open PRs=11, language=TypeScript, license=Apache-2.0"
  - "[GH:languages] GitHub languages API snapshot 2026-07-21: TypeScript=3154668, Python=2101321, JavaScript=190746, CSS=114844, Dockerfile=7299, Shell=5405"
  - "[GH:community] GitHub community/profile and contributors endpoints queried 2026-07-21: health_percentage=75, first-page contributors=18, Code of Conduct/CONTRIBUTING/PR template/license/readme detected"
  - "[GH:security] GitHub repository security-advisories API queried 2026-07-21: returned [] in this check"
  - "[GH:release] GitHub releases API snapshot 2026-07-21: 66 release objects; latest v0.9.0-beta published 2026-07-13; v0.8.9-beta added MCP auth and DeepSeek; v0.9.0-beta expanded template-v2 editing, grid/layout, charts/tables/text/font controls and removed v1 presentation/template endpoints"
  - "[GH:readme] README.md local scan at be3c083ac7875db9a1919e9bc155e033fadab999 on 2026-07-21: positions Presenton as open-source AI presentation generator/API with Docker, Electron, BYOK, local providers, editable PPTX/PDF export, template generation and built-in MCP"
  - "[Docs] https://docs.presenton.ai/ extracted 2026-07-21: documentation distinguishes Presenton Cloud and Open Source and describes developer/team presentation workflows"
  - "[Docs:quickstart] https://docs.presenton.ai/v3/get-started/quickstart extracted 2026-07-21: official GHCR Docker command maps host 5000 to container 80 and mounts ./app_data:/app_data"
  - "[Docs:api] https://docs.presenton.ai/v3/get-started/api-introduction and https://docs.presenton.ai/llms.txt extracted 2026-07-21: current index exposes versioned v0.9.0-beta Open Source docs, API families for files/images/presentations/themes/templates/async tasks/webhooks, MCP tutorial and PPTX/PDF/PNG export reference"
  - "[Docs:flow] https://docs.presenton.ai/v3/contribution-guides/presentation-generation-flow extracted 2026-07-21: generation flow maps upload/decompose/update files, create/stream outlines, prepare/stream presentation, update and export endpoints"
  - "[GH:local-scan] Local shallow clone /opt/data/tmp/github-repo-wiki-freshness-audit/evidence/repos/presenton at be3c083ac7875db9a1919e9bc155e033fadab999: git ls-files=10044, markdown/MDX=17, workflows=4, test-ish paths=69; root package plus servers/fastapi, servers/nextjs and electron directories"
  - "[GH:docker] Dockerfile and docker-compose.yml local scan 2026-07-21: Python/Node/nginx/Chromium/LibreOffice/Tesseract/LiteParse/Mem0/FastEmbed stack; production compose defaults host HTTP port 5001, maps Codex OAuth callback 1455, supports GPU profiles and exposes many model/search/image/auth env vars"
  - "[GH:fastapi] servers/fastapi/pyproject.toml checked 2026-07-21: Python 3.11 line, FastAPI, async DB drivers, mem0ai, fastmcp, multiple model providers, document parsers and pytest/coverage tooling"
  - "[GH:ci] .github/workflows/test-all.yml checked 2026-07-21: repository tooling tests/export verification, full locked FastAPI pytest, Next.js tests/lint/build and Cypress component tests are configured; this audit did not execute the workflow"
  - "[GH:auth] servers/fastapi/utils/simple_auth.py and get_env.py checked 2026-07-21: local auth uses PBKDF2-SHA256 with 200k iterations, HMAC-signed expiring session tokens/cookies, optional DISABLE_AUTH and env/user config; this is source review, not penetration testing"
  - "[GH:mcp] servers/fastapi/mcp_server.py checked 2026-07-21: MCP is disabled in Electron; server/Docker mode exposes only generate/generate_async/status routes, binds 127.0.0.1:8001, verifies Presenton session bearer tokens when app auth is configured, forwards auth to local API, and has 600s upstream timeout"
  - "[GH:contributing] CONTRIBUTING.md and CODE_OF_CONDUCT.md checked 2026-07-21: contributions outside electron/ may not be accepted; CLA required, unsigned PRs older than 30 days may be closed; AI-assisted PRs are allowed if disclosed and reviewed"
  - "[GH:vision] VISION.md checked 2026-07-21: frames Presenton as local/private, model-agnostic visual document engine with templates/design as first-class assets"
  - "[GH:tests] servers/fastapi/tests/test_presentation_generation_api.py checked 2026-07-21: tests cover request validation and mocked sync/async/status handler paths; no full generation-quality benchmark was run"
...
> **核验版本**: local clone commit `be3c083ac7875db9a1919e9bc155e033fadab999` / release `v0.9.0-beta`；GitHub/API 快照 2026-07-21
...
但保守看，当前项目仍很年轻：latest release 仍是 `v0.9.0-beta`，代码面横跨 FastAPI、Next.js、Electron、Docker、LibreOffice、Chromium、Tesseract、LiteParse、Mem0、MCP、多模型与 web-search provider。能力面广，亦即故障面广；本轮未启动 Docker 或生成 PPTX，因此推荐度保持 3 [GH:api][GH:release][GH:docker]。
...
3. **模型选择灵活**：README/compose 覆盖 OpenAI、Gemini、Vertex、Azure、Bedrock、Anthropic、DeepSeek、LM Studio、Ollama、OpenAI-compatible 等 provider，并加入多种 web-search/image provider 配置 [GH:readme][GH:docker][GH:release]。
...
1. **成熟度低**：项目创建于 2025-05，latest release 为 `v0.9.0-beta`，template API 还在从 v1 向 v2 收敛 [GH:api][GH:release]。
...
4. **贡献边界仍有治理张力**：CONTRIBUTING 过去限制 electron/ 外贡献；v0.9.0 又加入 CLA/PR age policy。开放贡献与产品快速重构之间仍需持续观察 [GH:contributing][GH:release]。
...
4. 文档列出 PPTX、PDF、PNG 导出路径；本轮只验证到文档/API reference，未实际导出 [Docs:api]。
5. 支持模板、smart design、图片上传、webhook、async task。
6. 支持多 LLM provider、BYOK、Ollama/OpenAI-compatible endpoint。
7. 在 server/Docker 模式通过 FastMCP 暴露 generate/generate_async/status 三个受限工具；配置应用认证时复用 session bearer token，Electron 中禁用 MCP [GH:mcp]。
...
| 场景 | CPU | 内存 | 存储 | 说明 |
|------|-----|------|------|------|
| Docker 最小试用 | 启发式：2-4 cores | 启发式：4GB+ | 启发式：多 GB image/runtime | 官方 quickstart 提供 Docker 路径；当前 compose 默认 host port 5001，具体命令与版本需一致 [Docs:quickstart][GH:docker] |
...
本地扫描显示它已具备真实工程结构：10044 个 tracked files、4 个 workflows、69 个 test-ish paths，覆盖 FastAPI、Next.js、Electron、Docker、auth、MCP，而不是单文件 demo [GH:local-scan]。FastAPI pyproject 有 pytest/coverage 配置，CI 会 build FastAPI binary 和 Next.js [GH:fastapi][GH:ci]。

保守点：仓库文件数近万，servers/、electron/、scripts/、export runtime、cached assets 混合；虽然当前 CI 配置覆盖 FastAPI、Next.js 与 Cypress，CONTRIBUTING 仍限制 electron/ 外贡献并加入 CLA/PR-age policy。代码可读性与长期维护成本都还需时间检验 [GH:ci][GH:contributing]。
...
| 维度 | 评分 | 说明 |
|------|------|------|
| 社区活跃度 | 3/5 | 9,122 stars、1,427 forks、first-page contributors=18、22 open issues、11 open PRs；有关注度，但不能据此推断生产采用 [GH:api][GH:community]。 |
| 成熟度 | 2/5 | 2025-05 创建，当前仍为 `v0.9.0-beta`，template/API/editor 面快速重构；不宜视为稳定生产基础设施 [GH:api][GH:release]。 |

## 安全与风险

评分 3/5。

积极面：本轮 repository advisory API 返回空；代码中存在 basic/session auth，密码使用 PBKDF2-SHA256，session cookie httponly，HTTPS 时 secure。MCP server 现在只映射三个 presentation workflow routes，在 app auth 配置时验证 session bearer token、默认绑定 127.0.0.1，并在 Electron 中禁用 [GH:security][GH:auth][GH:mcp]。这些结果仍不覆盖依赖审计、容器 CVE、运行时 hardening、文件解析 CVE 或 private advisories。
