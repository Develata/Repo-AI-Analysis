...
推荐度保守给 4，但只限于“本地可信、轻量 LLM ingestion”的条件：它很适合作为默认候选之一，尤其适合“先把材料变成 Markdown，再交给 LLM/agent”的场景；但它不该替代专门的 OCR/layout/document intelligence 工具。对复杂数学论文、扫描件、表格/公式密集 PDF，要和 Docling、MinerU、Marker 做实测后再决定。
...
local scan 显示包结构清楚，核心 converter registry、各格式 converter、MCP package、tests、CI workflows、SECURITY、CODE_OF_CONDUCT 都存在 [GH:local-scan]。v0.1.6 还修了 PDF conversion 的 O(n) memory growth 问题，说明维护者关注资源问题 [GH:release]。不给 5：本次未跑完整测试/benchmark；SUPPORT.md 仍有模板 TODO；open PR 数较高，说明治理压力不小 [GH:api][GH:local-scan]。
...
| 维度 | 评分 | 说明 |
|------|------|------|
| 社区活跃度 | 4/5 | 131k stars、9k forks、78 contributors first page、Microsoft 背景，关注度极高；但 open issues/PR 都不少，不能直接给满分 [GH:api][GH:contributors]。 |
