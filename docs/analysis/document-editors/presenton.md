...
sources:
  ...
  - "[GH:quarto] https://github.com/quarto-dev/quarto-cli — same-category comparison; existing local analysis under document-editors/quarto-cli.md"
  - "[GH:typst] https://github.com/typst/typst — same-category comparison; existing local analysis under document-editors/typst.md"
  - "[GH:overleaf] https://github.com/overleaf/overleaf — same-category comparison; existing local analysis under document-editors/overleaf.md"
---
...
它最适合纳入 wiki 的原因，是它补上了 document-editors 里“AI presentation generation / visual document engine”这一类：与 Typst/Quarto/Overleaf 的 text-first 写作不同，Presenton 关注 slide deck、模板、视觉资产、PPTX/PDF export 和 API 集成。
...
**是，但建议定位为“观察候选”而非“强推荐工具”**。

它适合写成一条短分享：“开源 AI presentation generator 开始从 demo 走向 API/self-host/MCP 形态”。但若面向普通读者，需明确 beta、部署重、生成质量未实测，避免把它宣传成成熟 Gamma replacement。
