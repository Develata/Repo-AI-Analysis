...
n8n 的主分类仍应是 `automation`：它是 workflow automation / integration platform；但在 2026 的语境下，它也已经是重要的 AI workflow 与 agent platform 边界项目。

## 总体评价

n8n 是一个大型、成熟、生态极强的 workflow automation 平台。它的核心不是“单个 agent”或“agent infrastructure primitive”，而是把触发器、节点、凭据、数据转换、外部 API、数据库、队列、人工审批、日志和部署治理组织成可视化 workflow [GH][Docs:home]。因此本次归入 `automation`，而不是 `agent-programs/agent-infrastructure`。
...
上述项目按 `automation` 同类范围做定位级对比，未按同一 10 维度框架深审。n8n 与 Dify 这类 LLM / agent application platform 有交叉，但 Dify 在本 wiki 中归入 `agent-programs/agent-frameworks`，不放入本节同类对比表。
