...
evidence: "official GitHub API/release/advisory metadata + current README/docs; prior code review retained. Test suite and real-time discussion response metrics not independently re-measured."
status: active
...
sources:
  ...
  - "[GH:API] https://api.github.com/repos/jdx/mise checked 2026-07-13 UTC+8; archived=false, default_branch=main, language=Rust, license=MIT, stars=30671, forks=1269, open PRs=14; repository Issues are disabled and support is routed to Discussions"
  - "[GH:release] https://github.com/jdx/mise/releases/tag/v2026.7.5 published 2026-07-09"
  - "[GH:advisories] https://api.github.com/repos/jdx/mise/security-advisories checked 2026-07-13 UTC+8; six published advisories include GHSA-fjj5-v948-whjj Critical, GHSA-436v-8fw5-4mj8 High, GHSA-77g9-363w-rccq High, and three Moderate advisories"
  - "[GHSA:mise] https://github.com/jdx/mise/security/advisories; fixes recorded by API: 2026.3.10 (GHSA-fjj5), 2026.6.1 (GHSA-f94h), 2026.6.4 (GHSA-29hf and GHSA-77g9 metadata), 2026.7.1 (GHSA-9mm4); GHSA-77g9 API reports vulnerable <=2026.6.4 and patched 2026.6.4, so branch/version semantics require advisory-level checking"
  - "[Docs] https://mise.en.dev"
  - "[Docs:security] https://mise.en.dev/dev-tools/security.html"
...
mise 是 asdf 的精神继承者，用 Rust 重写后补齐了 asdf 缺失的几乎所有维度：性能（无 shim 开销）、安全性（签名验证）、易用性（模糊匹配、单命令安装）和功能广度（内置 env 管理和 task runner）。2023 年 1 月创建，不到三年半已积累 30k+ stars，并保持极为活跃的 release 节奏 [GH:API] [GH:release]。
...
3. **重视供应链安全**。内置 Cosign/SLSA/Minisign/GitHub attestation 验证（主要覆盖 aqua backend 和 release artifacts），逐步将 asdf 插件迁移到作者控制的 org，减少对第三方插件代码的信任依赖 [Docs] [Docs:security]
...
4. **配置格式仍在演进**。例如 `env_file` 已被标记为 deprecated，改用 `env` table + `_.file`
...
**环境变量管理**：在 `mise.toml` 中声明式管理 environment variables，自动加载 `.env` 文件，进入项目目录时自动设置/退出时自动清理 [Docs]。

**任务执行**：定义多步骤任务，支持依赖编排、文件监听、内联脚本。替代 `make`/`npm scripts` [Docs]。
...
**任务系统**支持 watch-files 配置监听文件变更触发任务、任务依赖、内联脚本和自定义 shell [Docs]。
...
| 维度 | 评分 | 说明 |
|------|------|------|
| 社区活跃度 | 4/5 | 2026-07-13 GitHub API 显示 30,671 stars、1,269 forks；Discord / Discussions 活跃，但贡献高度集中在 jdx，bus factor 仍低 [GH:API] |
...
Nix 生态（4k+ open issues）的反面：mise 仓库 Issues 当前关闭，主要把支持与 bug report 导向 Discussions；本次 API 快照只看到 14 个 open PR。社区健康度看起来远好于 issue 堆积的项目，但这种治理方式也意味着“open issue count”不能直接作为 backlog 指标 [GH:API]。

## 安全与风险

评分 3/5。

mise 在同类工具中明显重视安全，但 2026-07 的公开 advisory 数量使评分必须更保守：

- **签名验证**：Cosign、SLSA、Minisign、GitHub attestation 多种签名验证机制内建 [Docs] [Docs:security]
...
- **公开 advisory**：本次 GitHub advisory API 返回 6 条 published advisories，覆盖 Critical / High / Moderate 等等级；其中多条与信任配置、签名/校验边界或执行路径有关。多数已有修复版本，但这说明 mise 的安全面不是理论风险 [GH:advisories] [GHSA:mise]

**风险点**：
- 单一维护者主导，jdx 个人账号安全是整个项目安全的单点
- 签名验证主要覆盖 aqua backend 和 release artifacts，非所有 backend/工具路径
- 快速迭代意味着每次 `mise upgrade` 都在运行新代码，理论上增加供应链攻击窗口（签名验证只能缓解部分路径）
- GitHub advisory API 对 GHSA-77g9 的 patched-version 元数据呈现出“vulnerable <=2026.6.4 / patched 2026.6.4”的边界，具体操作前应读 advisory 细节，而不是只看摘要 [GHSA:mise]
