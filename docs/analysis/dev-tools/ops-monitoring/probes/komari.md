...
evidence: "official GitHub API/release/advisory metadata + current README/docs/current-issue sample + prior local source scan. Local Go tests and LOC were not rerun."
archived_reason: ""
docker_support: true
gpu_required: false
...
sources:
  ...
  - "[GH:API] https://api.github.com/repos/komari-monitor/komari checked 2026-07-13 UTC+8; archived=false, default_branch=main, language=Go, license=MIT, stars=5323, forks=505, open issues=64, open PRs=0"
  - "[GH:README] https://github.com/komari-monitor/komari/blob/main/README.md"
  - "[GH:release] https://github.com/komari-monitor/komari/releases/tag/1.2.6 published 2026-07-12; metric-store/RPC identity/security changes and removal of built-in Cloudflare Tunnel/Access"
  - "[GH:advisories] https://api.github.com/repos/komari-monitor/komari/security-advisories checked 2026-07-13 UTC+8; GHSA-hxjg-93wc-h8p8 High (<=1.2.0, patched 1.2.2), plus two 2025 High advisories"
  - "[GH:issues] GitHub GraphQL current open-issue sample checked 2026-07-13 UTC+8; 64 open issues / 0 open PRs; sampled #595 PostgreSQL DSN mismatch, #594 removed reverse proxy question, #568 node-data fetch failure, #585 unauthenticated admin-page exposure"
...
主要扣分点也同样清楚：项目很年轻（2025-04 创建，2026-07 最新 v1.2.6），代码与治理仍由核心维护者强主导；本次 GitHub GraphQL 快照显示 64 个 open issues / 0 个 open PR，近期仍能看到数据库 DSN、节点数据获取、反代与未授权访问边界等问题报告 [GH:API] [GH:issues]。同时它暴露 admin API、API Key、OAuth/2FA、远程终端、RPC/agent 上报等能力，便利性与攻击面同步上升。
...
5. **社区增长快**。截至 2026-07-13 GitHub API 显示 5,323 stars、505 forks；open PR 为 0，latest release 仍在 2026-07 发布，说明项目保持活跃迭代 [GH:API] [GH:release]

## 劣势

1. **项目年轻，稳定性仍需观察**。2025-04 创建，虽已到 v1.2.6，但 open issues 中仍有数据库、反代、节点数据获取、访问边界等具体 bug / support 信号 [GH:API] [GH:issues]
...
| 维度 | 评分 | 说明 |
|------|------|------|
| 社区活跃度 | 4/5 | 2026-07-13 GitHub API 显示 5,323 stars、505 forks、open PR 0；latest release `1.2.6` 发布于 2026-07-12，说明社区和维护仍活跃，但 closed PR 数不单独等价于高治理质量 [GH:API] [GH:release] |
| 成熟度 | 3/5 | 2025-04 创建，2026-07 已有 `1.2.6`；但项目年龄短，open issues 仍有数据库/网络/访问边界等问题，稳定性未到长期验证阶段 [GH:API] [GH:issues] |
...
1. **攻击面不小**。admin API、remote terminal、任务调度/下发、agent token、WebSocket/RPC 都需要严肃边界控制；v1.2.6 移除了内建 Cloudflare Tunnel / Access 相关能力，这是收缩集成面的正面变化，但不消除 Web 管理面本身的风险 [GH:release] [GH:source]
...
4. **公开 advisory 已存在**。本次 GitHub repository advisory 检查看到 2026-06 的 High advisory（影响 `<=1.2.0`、修复于 `1.2.2`）以及两条 2025 High advisories；这些不等于当前最新版仍暴露，但说明管理面板和 agent/RPC 边界必须认真升级维护 [GH:advisories]
5. **依赖面中等**。`go.mod` 包含 Gin、Gorm、SQLite driver、OIDC、goja、gRPC、x/crypto/x/net 等依赖；不是极简二进制 [GH:source]
